...
\item Calcular la recta tangente y el plano normal a la trayectoria
      ...
      \begin{indication}
      \begin{enumerate}
      \item Introducir el comando \command{f(t):=(sen(t),cos(t),sqrt(t))} en la \field{Barra de Entrada} de la \field{Vista CAS} y activar a \field{Vista Gráficas 3D}.
...
\item Dada la función $f(x,y)=20-4x^2-y^2$, se pide calcular en el punto $(2,-3)$:
      ...
      \item Vector gradiente.
            ...
            \item Para calcular la derivada parcial con respecto a $x$, introducir el comando \command{f'\_x(x,y):=Derivada(f, x)} en la \field{Barra de Entrada}.
            \item Para calcular la derivada parcial con respecto a $y$, introducir el comando \command{f'\_y(x,y):=Derivada(f, x)} en la \field{Barra de Entrada}.
