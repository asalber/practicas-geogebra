...
\item Calcular las siguientes integrales:
      ...
      \item $ \int{x^2 \log x\,dx}$
            \begin{indication}
            Introducir el comando \command{Integral(x\^{}2log(x))} en la barra de \field{Entrada} de la \field{Vista CAS}.
            \end{indication}

      \item $\displaystyle \int \frac{\log(\log x)}{x}\,dx$
            \begin{indication}
            Introducir el comando \command{Integral(log(log(x)))} en la barra de \field{Entrada} de la \field{Vista CAS}.
            \end{indication}

      \item $\displaystyle \int \frac{5x^{2}+4x+1}{x^{5}-2x^{4}+2x^{3}-2x^{2}+x}\,dx$
            \begin{indication}
            Introducir el comando \command{Integral((5x\^{}2+4x+1)/(x\^{}5-2x\^{}4+2x\^{}3-2x\^{}2+x))} en la barra de \field{Entrada} de la \field{Vista CAS}.
            \end{indication}

      \item $\displaystyle \int \frac{6x+5}{(x^{2}+x+1)^{2}}\,dx$
            \begin{indication}
            Introducir el comando \command{Integral((6x+5)/((x\^{}2+x+1)\^{}2))} en la barra de \field{Entrada} de la \field{Vista CAS}.
            \end{indication}
      \end{enumerate}
...
\item Calcular las siguientes integrales definidas y representarlas gráficamente:
      ...
      \item $\displaystyle \int_{-\frac{1}{2}}^0 \frac{x^{3}}{x^{2}+x+1}\,dx$
            \begin{indication}
            \begin{enumerate}
            \item Introducir la función \command{f(x):=x\^{}3/(x\^{}2+x+1)} en la barra de \field{Entrada} de la \field{Vista CAS} y activar la \field{Vista Gráfica}.
      ...
      \item $\displaystyle \int_2^4 \frac{\sqrt{16-x^{2}}}{x}\,dx$
            \begin{indication}
            \begin{enumerate}
            \item Introducir la función \command{g(x):=sqrt(16-x\^{}2)/x} en la barra de \field{Entrada} de la \field{Vista CAS}.
      ...
      \item $\displaystyle \int_0^{\frac{\pi}{2}} \frac{dx}{3+\cos(2x)}$
            \begin{indication}
            \begin{enumerate}
            \item Introducir la función \command{h(x):=1/(3+cos(2x))} en la barra de \field{Entrada} de la \field{Vista CAS}.
...
\item Calcular la siguiente integral impropia $\int_2^{\infty} x^2e^{-x}\,dx$ y representarla gráficamente.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{f(x):=x\^{}2 exp(-x)} en la barra de \field{Entrada} de la \field{Vista CAS} y activar la \field{Vista Gráfica}.
...
\item Representar la parábola $y=x^{2}-7x+6$, y calcular el área limitada por dicha parábola, el eje de abscisas y las rectas $x=2$ y $x=7$.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{f(x):=x\^{}2-7x+6} en la barra de \field{Entrada} de la \field{Vista CAS} y activar la \field{Vista Gráfica}.
...
\item Calcular y dibujar el área comprendida entres las funciones $\sen x$ y $\cos x$ en el intervalo $[0,2\pi]$.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{f(x):=sen(x)} en la barra de \field{Entrada} de la \field{Vista CAS} y activar la \field{Vista Gráfica}.
      \item Introducir la función \command{g(x):=cos(x)} en la barra de \field{Entrada}.
      \item Introducir el comando \command{Integral(abs(f(x)-g(x)), 0, 2pi)} en la barra de \field{Entrada} y hacer clic en el botón de \button{Valor numérico}.
...
\item Representar gráficamente la región del primer cuadrante limitada por la función $f(x)=(x+\sen x)^2$, la recta $x=2\pi$ y el eje $OX$, y hallar el volumen generado en la rotación alrededor del eje $OX$ de la región anterior.
      \begin{indication}
      \begin{enumerate}
      \item Introducir la función \command{f(x):=(x+sen(x))\^{}2} en la barra de \field{Entrada} de la \field{Vista CAS} y activar la \field{Vista Gráfica}.
      \item Para calcular el área plana introducir el comando \command{Integral(f(x), 0, 2pi)} en la barra de \field{Entrada} y hacer clic en el botón de \button{Valor numérico}.
      \item Para representar gráficamente el área, hacer clic en el círculo que aparece a la izquierda de la expresión anterior.
