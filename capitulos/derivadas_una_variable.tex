...
botón \button{Calcular un límite}. Por lo tanto, si existe la derivada en $x=1$ su valor es:
\[
f'(1) = \mathop {\lim }\limits_{h \to 0} \frac{{f(1 + h) - f(1)}}
{h}
\]
\item Para la función $g(x)$, podemos, para su definición, utilizar la función condicional de Derive \command{If(condición,opción 1,opción
2)}, de tal forma que si se cumple la \command{condición} el programa realizará la \command{opción 1}, y si no se cumple realizará la
\command{opción 2}. La función condicional de Derive, \command{If}, entre otras muchas posibilidades, sirve para introducir funciones
definidas a trozos. En nuestro caso la \command{condición} es $x\neq y$, la \command{opción 1} es $x\sin(1/x)$, y la \command{opción 2} es
$0$.
...
\begin{enumerate}
\item Definir la función en la venta de Álgebra con el nombre \command{f(x)}.
\item Para la primera derivada introducir la expresión \command{f'(x)} y hacer clic en el botón \button{Simplificar}.
\item Para la segunda derivada introducir la expresión \command{f''(x)} y hacer clic en el botón \button{Simplificar}.
\item Para la tercera derivada introducir la expresión \command{f'''(x)} y hacer clic en el botón \button{Simplificar}.
\item Para la cuarta derivada introducir la expresión \command{f''''(x)} y hacer clic en el botón \button{Simplificar}.
\end{enumerate}
\end{indicacion}
...
\item Hacer clic en el botón \button{Representar Expresión} de la ventana gráfica.
\item Introducir la expresión \command{f(1)+f'(1)(x-1)}, correspondiente a la ecuación de la recta tangente a la gráfica de la función $f$ en $x=1$, en la ventana de Álgebra y hacer clic en el botón \button{Simplificar}.
\item Hacer clic en el botón \button{Representar Expresión} de la ventana gráfica.
\end{enumerate}
\end{indicacion}
...
\begin{indicacion}
Una vez introducida la función en Derive, utilizar el botón \button{Ventana 2D} para pasar al entorno de dibujo de funciones de una única
variable, y allí utilizar el botón \button{Representar Expresión} para representar la gráfica. 
\end{indicacion}

\item  Calcular la función derivada $g^{\prime }(x),$ y representar su
gráfica.
\begin{indicacion}
Marcar la expresión de la función y utilizar el botón \button{Hallar una derivada} y escoger la variable $x$ y el orden 1. Para representar
la gráfica de la función derivada, seguir el proceso del punto anterior.
\end{indicacion}

\item  Calcular las raíces de $g^{\prime }(x).$
\begin{indicacion}
Para calcular las raíces, marcar la expresión de la función derivada y utilizar el botón \button{Resolver o despejar}, escogiendo la variable
...
\begin{indicacion}
La podemos obtener derivando, mediante el botón \button{Hallar una derivada}, la función de partida, y escogiendo como orden de derivación 2;
...
\begin{indicacion}
Igual que en apartados anteriores, marcamos la expresión de la segunda derivada y utilizamos el botón \button{Resolver o despejar}.
\end{indicacion}
