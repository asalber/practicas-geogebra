...
representarla, utilizar el botón \button{Ventana 3D}, y una vez en el
entorno tridimensional, utilizar el botón \button{Representar}.

\item

\begin{itemize}

\item Para rotarla, utilizar los botones \button{Rota la gráfica},
\button{Girar hacia la izquierda}, \button{Girar hacia la derecha},
\button{Rotar hacia arriba} y \button{Rotar hacia abajo}.

\item Para agrandarla y contraerla utilizar los botones
\button{Magnificar} y \button{Contraer}. Observar cómo, tanto al
...
obtener mediante el botón \button{Fijar la posición del ojo}.

\item Para hacer zoom hacia fuera o hacia dentro utilizar los
botones \button{Zoom hacia fuera} y \button{Zoom hacia dentro}.
...
efecto similar se obtiene con los botones \button{Ajuste del
mínimo/máximo} y \button{Restablece rango}.
...
botón \button{Ventana 2D}. Recordar que las curvas de nivel son
...
\end{indication}
  ...
  \item Utilizar la función \command{VECTOR(u,k,m,n)} para dibujar todas las
  curvas de nivel de la forma $f(x,y) = 1/k$, donde $k$ toma los
  valores naturales de $2$ a $10$ (tener en cuenta que, en la función \command{Vector},
...
\item La función \command{VECTOR} de Derive, genera un
...
\begin{center}
\command{VECTOR}$(f(x,y)=1/k,k,2,10)$
\end{center}
...
procedemos a representar en la \button{Ventana 2D} de igual forma que
...
\button{Magnificar} y los de \button{Rotación} (hacia arriba y hacia
abajo) y de \button{Giro} (a izquierda y derecha), además de cambiar
...
\button{Calcular un límite}) de la función inicialmente considerando
...
resultante (botón \button{Calcular un límite}). Observar que la
...
función condicional de Derive \command{If(condición,opción 1,opción
2)}, de tal forma que si se cumple la \command{condición} el
programa realizará la \command{opción 1}, y si no se cumple
realizará la \command{opción 2}. La función condicional de Derive,
\command{If}, entre otras muchas posibilidades sirve para introducir
funciones definidas a trozos. En nuestro caso la \command{condición}
es $x<0$, la \command{opción 1} es $0$, y la \command{opción 2} es
$\cos(x^2+y^2)$.
...
\begin{indication}
{Para representar la gráfica, botón \button{Ventana 3D}, y una vez en
el entorno de dibujo de funciones de dos variables, botón
\button{Representar}. Convendrá editar la gráfica (botón derecho del
...
utilizar la función condicional \command{If}, y para introducir la
\command{condición} $(x,y)=(0,0)$ utilizar $x=0$ \command{and}
$y=0$).
\end{enumerate}